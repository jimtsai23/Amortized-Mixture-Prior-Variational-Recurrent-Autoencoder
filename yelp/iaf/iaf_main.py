...
latent_dim = 32
dropout_rate = 0.5
model = afpm(vocab_size=datasets['train'].vocab_size,
...
for ep in range(epoch):
    ...
    for split in splits:
        ...
        for itr, (enc_inputs, dec_inputs, targets, lengths, labels) in enumerate(dataloader):
            ...
            # forward
            logp, z_0, z_T, mu, logvar, labels = model(enc_inputs, dec_inputs, lengths, labels)
...
if(True):
    ...
    for itr, (enc_inputs, dec_inputs, _, lengths, labels) in enumerate(dataloaders['test']):
        ...
        _, z_0, z_T, mu, _, labels = model(enc_inputs, dec_inputs, lengths, labels)
...
tsne_z = TSNE(n_components=2,perplexity=200).fit_transform(features)#early_exaggeration=100
tracker['z'] = tsne_z
