...
latent_dim = 32
dropout_rate = 0.5
model = ampm(vocab_size=datasets.vocab_size,
...
a = 0
b = 0
features = np.empty([5933, latent_dim])
feat_label = np.empty(5933)
for i in range(16):
    b += n_sent[i]
    print(a,b)
    data = datasets[a:b]
    ...
    features[a:b] = mu.data.cpu().numpy()
    feat_label[a:b] = labels.cpu().numpy()
...
sio.savemat("ampm_summ.mat", tracker)
