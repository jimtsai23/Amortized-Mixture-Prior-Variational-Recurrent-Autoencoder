...
pseudo_dataset = datasets['valid'][1000:2000]#[:pseudo_size]
...
for ep in range(epoch):
    ...
    for split in splits:
        ...
        for itr, (enc_inputs, dec_inputs, targets, lengths, labels) in enumerate(dataloader):
            ...
            # forward
            logp, z_q, mu, logvar, logc, labels = model(enc_inputs, dec_inputs, lengths, labels)
...
with torch.no_grad():
    ...
    for itr, (enc_inputs, dec_inputs, targets, lengths, labels) in enumerate(dataloaders['test']):
        ...
        # forward
        logp, z_q, mu, logvar, _, _ = model(enc_inputs, dec_inputs, lengths, labels)
...
with torch.no_grad():
    ...
    for itr, (enc_inputs, dec_inputs, targets, lengths, labels) in enumerate(dataloaders['test']):
        ...
        # forward
        logp, z, mu, logvar, _, _ = model(enc_inputs, dec_inputs, lengths, labels)
...
if(True):
    ...
    for itr, (enc_inputs, dec_inputs, _, lengths, labels) in enumerate(dataloaders['test']):
        ...
        lengths = lengths.to(device)
        labels = labels.to(device)
        _, _, mu, _, _, labels = model(enc_inputs, dec_inputs, lengths, labels)
